...
from user import jwt_exists, jwt_match
...
"""
    ...
    def get(self, isbns):
        ...
        Args:
                isbns (str[]): A list of isbns to query with '+jwt'
        ...
        jwt = isbns[1]
        all_isbns = isbns[0]
        if not jwt_exists(jwt):
            return {"message": "begone hacker! invalid jwt - line 255 book.py"}
        all_isbns = all_isbns.split(",")
    ...
    def post(self, isbns):
        ...
        Args:
                isbns (str): The isbn of the book being posted (jwt in body)

        Returns:
                message: What happened with the post call.
        """
        data = Book.parser.parse_args()
        if not jwt_exists(data["jwt"]):
            return {"message": "begone hacker! invalid jwt - line 276 book.py"}
    ...
    #def delete(self, isbns):
        """Deletes a book from the database. Currently inactive
...
"""
    ...
    def get(self, search):  # books -> users
        ...
        Args:
                search (str): The string to search with '+jwt'.
        ...
        token = search[1]
        search = search[0]
        if not jwt_exists(token):
            return {"message": "begone Hacker! jwt invalid - line 329 book.py"}
