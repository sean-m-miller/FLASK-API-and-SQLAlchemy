...
class Listing(Resource):
    ...
    def get(self, ids):  # get request, looking for all listing objects from listing ID's, user -> books
        ...
        ids = ids[0]
        if jwt_exists(token):
            return {"message": "begone hacker! invalid jwt - line 245 listing.py"}
        ids = ids.split(",")
    ...
    def post(self, ids):
        ...
        if not jwt_match(data['google_tok'], data["jwt"]):
    ...
    def delete(self, ids):
        ...
        data = Listing.parser.parse_args()
        if not jwt_match(data['google_tok'], data["jwt"]):
            return {"message": "begone hacker! your jwt does not match your google token"}, 403
        id_ = int(ids)
    ...
    def put(self, ids): #
        """Either posts listing to database, or updates it. Currently inactive
        ...
        if not jwt_matches(data['google_tok'], data["jwt"]):
            return {"message": "begone hacker! your jwt does not match your google token"}
        if(listing_id):
...
class allListings(Resource):
    ...
    def get(self, search):
        ...
        search = search.split("+")
        if not jwt_exists(search[len(search)-1]):
            return {"message": "begone hacker! invalid jwt - line 381 listing.py"}
        strings = search[:len(search)-1] #get rid of the last index
        if search[0] == "home":  # right now, it just returns the most recent listings, not most recent 20 books
